...
def run_forecast(df, pred_len=4, sample_count=30):
    ...
    pred_df = predictor.predict(
        df=x_df, x_timestamp=x_ts_naive, y_timestamp=y_ts,
...
def build_chart(df, pred_df, future_ts):
    ...
    connect_x = [19, 20]
    connect_y = [actual_close[-1], mean_close[0]]
    ax.plot(connect_x, connect_y, color='orange', linewidth=2, linestyle='--')
    forecast_x = range(20, 20 + len(mean_close))
    ax.plot(list(forecast_x), mean_close, color='orange',
            linewidth=2.5, linestyle='--', label='Kronos Forecast')
    ax.fill_between(list(forecast_x), lower_close, upper_close,
                    alpha=0.25, color='orange', label='Confidence Band (15-85%)')
...
def build_table(df, pred_df, future_ts):
    ...
    bias = f"BULLISH ▲ +${final_mean - current_price:.2f}" \
           if final_mean > current_price \
           else f"BEARISH ▼ -${current_price - final_mean:.2f}"
    rows = []
    for ts, row in zip(future_ts, pred_df.itertuples()):
        rows.append({
            "time"  : ts.strftime('%H:%M ICT'),
            "open"  : round(row.open, 2),
            "high"  : round(row.high, 2),
            "low"   : round(row.low,  2),
            "close" : round(row.close_mean, 2),
            "lower" : round(row.close_lower, 2),
            "upper" : round(row.close_upper, 2),
        })
    return {
        "current_price" : round(current_price, 2),
        "bias"          : bias,
        "forecast_start": future_ts[0].strftime('%H:%M ICT'),
        "forecast_end"  : future_ts[-1].strftime('%H:%M ICT'),
        "rows"          : rows,
    }
...
def handler(job):
    ...
    try:
        ...
        pred_df, future_ts, _ = run_forecast(df, pred_len=pred_len,
